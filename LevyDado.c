...
/* Gerando todos os valores de ell e mu*/
	...
	for(index=11;index<31;index+=INDEX_INCREMENT){
		for(ell=0.1;ell<=10;ell/=ELL_INCREMENT){
			if(index == 30 && ell == 10){
				fprintf(arq1, "(alpha=%.2lf x ell=%.2lf)\n", index/10, ell);
			}
			else{
			fprintf(arq1, "(alpha=%.2lf x ell=%.2lf),", index/10, ell);
			}
		}
	...
	for(ell=0.1;ell<=10;ell/=ELL_INCREMENT){
		if(index == 30 && ell == 10){
			fprintf(arq1, "%lf \n", rng_levy48(index/10, ell));
		}
		else{

			fprintf(arq1, "%lf,", rng_levy48(index/10, ell));
		}
	}
	}
	}
